...
function initializeApp() {
    document.addEventListener('DOMContentLoaded', () => {
        try {
            ...
            showTab(DOM_IDS.DASHBOARD); // Default to Dashboard, not Import
...
function loadData() {
    ...
    [projects, archivedProjects].forEach(projectList => {
        projectList.forEach(project => {
            project.id = project.id || (projects.length + archivedProjects.length + 1).toString();
...
function parseCsv(text, overviewOnly = false) {
    ...
    lines.forEach((line, index) => {
        ...
        if (!currentProject || currentProject.id !== projectId) {
            if (currentProject && overviewOnly && currentProject.taskCompletions) {
                // Finalize previous project’s completion as average
                ...
                currentProject.completion = currentProject.taskCompletions.length > 0 
                    ...
                    : 0;
                delete currentProject.taskCompletions; // Clean up temporary array
            }
            currentProject = { 
                id: projectId, 
                name: projectName, 
                tasks: [], 
                pic: pic || '', 
                completion: 0, // Initial value, updated later in overview mode
                expanded: false, 
                calendarExpanded: false 
            };
            if (overviewOnly) {
                currentProject.taskCompletions = []; // Temporary array for averaging
            }
        ...
        if (overviewOnly) {
            // Collect task completions for averaging
    ...
    // Finalize the last project’s completion in overview mode
...
function getProjectCompletion(project) {
    ...
    // Archive project if it reaches 100% completion
    if (project.completion === 100) {
        const projectIndex = projects.findIndex(p => p.id === project.id);
        if (projectIndex !== -1) {
            const [archivedProject] = projects.splice(projectIndex, 1);
...
function filterProjects() {
    ...
    let filteredProjects = [...projects]; // Only active projects
...
function renderProjects() {
    filterProjects(); // Render only active projects
}
...
function showContextMenu(event, type, projectId, taskIdx = null, subIdx = null) {
    ...
    // Position the menu with a precise 2px Y offset below the cursor
    ...
    // Adjust if menu would overflow the viewport
    ...
    if (menuRect.bottom > viewportHeight) {
        contextMenu.style.top = `${event.clientY + window.scrollY - menuRect.height - 2}px`; // Move above cursor with 2px offset
    }
}
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            try {
                ...
                console.log('CSV Text:', text); // Debug: See raw CSV content
                importedProjects = parseCsv(text, true); // Load overview only
                console.log('Imported Projects:', importedProjects); // Debug: Check parsed data
                showTab('import'); // Switch to Import tab
                renderImportPage(); // Render after loading CSV
            } catch (error) {
        ...
        console.log('No file selected'); // Debug: Confirm file input
    }
}
...
function importSelectedProjects() {
    ...
    // Reload full CSV data for selected projects
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            try {
                ...
                // Clear existing projects and replace with selected ones
                projects = [];
                selectedProjects.forEach(project => {
                    projects.push(project);
                });
                
                // Clear temporary storage and save
                importedProjects = [];
                saveData();
                showTab(DOM_IDS.DASHBOARD); // Return to Dashboard
                populateFilterOptions();
            } catch (error) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            try {
                ...
                // Clear existing projects and replace with all from CSV
                projects = [];
