...
function initializeApp() {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (Notification.permission === "granted") {
            document.getElementById(DOM_IDS.NOTIFICATION_PERMISSION).style.display = 'none';
            checkNotifications();
        }
    });
...
function populateFilterOptions() {
    const filterPic = document.getElementById(DOM_IDS.FILTER_PIC);
    if (filterPic) {
...
function filterProjects() {
    const filterPic = document.getElementById(DOM_IDS.FILTER_PIC).value;
    const filterOverdue = document.getElementById(DOM_IDS.FILTER_OVERDUE).value;
    const filterCompletion = document.getElementById(DOM_IDS.FILTER_COMPLETION).value;
...
function renderFilteredProjects(filteredProjects) {
    const tbody = document.getElementById(DOM_IDS.PROJECT_LIST);
    if (!tbody) return;
    tbody.innerHTML = '';
...
function updateTabs() {
    ...
    tabsDiv.innerHTML = `
        ...
        <button class="tab-button ${openTabs[0] === 'gantt' ? 'active' : ''}" onclick="showTab('gantt')" role="tab" aria-selected="${openTabs[0] === 'gantt'}" aria-controls="gantt">Gantt Chart</button>
    `;
}
...
function showTab(tabId) {
    ...
    if (tabId === 'gantt' && selectedProjectId) {
...
function renderCalendar() {
    ...
    const monthYear = document.getElementById(DOM_IDS.MONTH_YEAR);
    if (monthYear) {
    ...
    if (calendarView === 'full') {
        const calendarFull = document.getElementById(DOM_IDS.CALENDAR_FULL);
        const calendarList = document.getElementById(DOM_IDS.CALENDAR_LIST);
...
function renderFullCalendar(monthStart, monthEnd) {
    const tbody = document.getElementById(DOM_IDS.CALENDAR_BODY);
    if (!tbody) {
...
function renderListCalendar(monthStart, monthEnd) {
    const list = document.getElementById(DOM_IDS.CALENDAR_LIST);
    if (!list) return;
    list.innerHTML = '';
...
function toggleCalendarView() {
    ...
    const toggleBtn = document.getElementById(DOM_IDS.TOGGLE_VIEW_BTN);
    if (toggleBtn) {
...
function renderGanttChart(projectId) {
    ...
    const tbody = document.getElementById(DOM_IDS.GANTT_BODY);
    const timelineHeader = document.getElementById(DOM_IDS.GANTT_TIMELINE);
    ...
    project.tasks.forEach((task, taskIdx) => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <div class="gantt-container">
                    <div class="task-bar" style="left: ${leftPercent}%; width: ${widthPercent}%; background: ${isOverdue ? '#ff0000' : '#004EA1'};" data-project-id="${projectId}" data-task-idx="${taskIdx}" data-sub-idx="null" data-total-days="${totalDays}" data-min-date="${minDate.toISOString().split('T')[0]}">
        ...
        task.subTasks.forEach((subTask, subIdx) => {
            ...
            subTr.innerHTML = `
                <td style="padding-left: 30px;" oncontextmenu="showContextMenu(event, 'ganttSubTask', '${projectId}', ${taskIdx}, ${subIdx})">
                    ${subTask.name}
                ...
                <td>
                    <div class="gantt-container">
                        <div class="task-bar" style="left: ${subLeftPercent}%; width: ${subWidthPercent}%; background: ${subIsOverdue ? '#ff0000' : '#004EA1'};" data-project-id="${projectId}" data-task-idx="${taskIdx}" data-sub-idx="${subIdx}" data-total-days="${totalDays}" data-min-date="${minDate.toISOString().split('T')[0]}">
...
function renderPICList() {
    const tbody = document.getElementById(DOM_IDS.PIC_LIST);
    if (!tbody) return;
    tbody.innerHTML = '';
...
function toggleTaskList() {
    const taskListPopup = document.getElementById(DOM_IDS.TASK_LIST_POPUP);
    ...
    const toggleBtn = document.getElementById(DOM_IDS.TOGGLE_TASK_LIST);
    if (toggleBtn) {
...
function renderTaskList() {
    const taskListContent = document.getElementById(DOM_IDS.TASK_LIST_CONTENT);
...
function showUndoButton() {
    const undoButton = document.getElementById(DOM_IDS.UNDO_BUTTON);
    if (undoButton) {
