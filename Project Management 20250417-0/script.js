...
function getProjectStatus(project) {
    if (getProjectCompletion(project) === 100) {
        return "Complete";
    }
    ...
    const allStartDates = allTasks.map(task => new Date(task.startDate).getTime());
    const allDueDates = allTasks.map(task => new Date(task.due).getTime());
    const minStart = new Date(Math.min(...allStartDates));
    const maxDue = new Date(Math.max(...allDueDates));
    const totalDuration = (maxDue - minStart) / (1000 * 60 * 60 * 24);
    const riskThreshold = totalDuration * 0.10;

    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const pastDue = allTasks.some(task => {
        const due = new Date(task.due);
        due.setHours(0, 0, 0, 0);
        return due < today && task.completion < 100;
    });
    if (pastDue) {
        return "Past Due";
    }

    const atRisk = allTasks.some(task => {
        const due = new Date(task.due);
        due.setHours(0, 0, 0, 0);
        const daysUntil = (due - today) / (1000 * 60 * 60 * 24);
        return daysUntil > 0 && daysUntil <= riskThreshold && task.completion < 100;
    });
    if (atRisk) {
        return "At Risk";
    }
...
function getTaskStatus(task, project) {
    // Check if task is complete
    if (task.completion === 100) {
        return "Complete";
    }

    // Calculate project total duration for risk threshold
    const allTasks = getAllTasks(project);
    if (allTasks.length === 0) {
        return "On Schedule"; // Fallback if no tasks exist
    }
    const allStartDates = allTasks.map(t => new Date(t.startDate).getTime());
    const allDueDates = allTasks.map(t => new Date(t.due).getTime());
    const minStart = new Date(Math.min(...allStartDates));
    const maxDue = new Date(Math.max(...allDueDates));
    const totalDuration = (maxDue - minStart) / (1000 * 60 * 60 * 24);
    const riskThreshold = totalDuration * 0.10;
    ...
    // Check for Past Due
    if (due < today && task.completion < 100) {
        return "Past Due";
    }

    // Check for At Risk
    const daysUntil = (due - today) / (1000 * 60 * 60 * 24);
    if (daysUntil > 0 && daysUntil <= riskThreshold && task.completion < 100) {
        return "At Risk";
    }
...
function renderFilteredProjects(filteredProjects) {
    ...
    filteredProjects.forEach(project => {
        ...
        const status = getProjectStatus(project).toLowerCase().replace(' ', '-');
        tr.classList.add('project-row', status);
...
function printScheduleFromGantt() {
    if (selectedProjectId) {
        printGanttSchedule(selectedProjectId);
    } else {
        alert('No project selected to print schedule for.');
    }
}
/*
 *************************************************************************
...
function setupContextMenu() {
    ...
    document.addEventListener('contextmenu', (e) => {
        ...
        if (projectId) {
            ...
            // Reset position styles and adjust for page offset
            contextMenu.style.position = 'absolute';
            const scrollX = window.scrollX || window.pageXOffset;
            const scrollY = window.scrollY || window.pageYOffset;
            const cursorX = e.clientX;
            const cursorY = e.clientY;
            const pageOffsetY = document.body.getBoundingClientRect().top; // Account for body offset
            const adjustedX = cursorX + scrollX;
            const adjustedY = cursorY + scrollY - pageOffsetY - 121.6; // Subtract consistent offset
...
function renderCalendar() {
    ...
    const monthYearSpan = document.querySelector(`[data-id="${DOM_IDS.MONTH_YEAR}"]`);
    if (!calendarFull || !calendarList || !tbody || !monthYearSpan) return;
    ...
    monthYearSpan.textContent = currentMonth.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });

    calendarFull.classList.toggle('hidden', calendarView !== 'full');
    calendarList.classList.toggle('hidden', calendarView !== 'list');

    let tasksByDay = {};
    projects.forEach(project => {
        if (filterProject && project.id !== filterProject) return; // Apply project filter
        project.tasks.forEach(task => {
            const dueDate = new Date(task.due);
            const dayKey = dueDate.toISOString().split('T')[0];
            if (dueDate.getMonth() === currentMonth.getMonth() && dueDate.getFullYear() === currentMonth.getFullYear()) {
                if (!tasksByDay[dayKey]) tasksByDay[dayKey] = [];
                tasksByDay[dayKey].push({ project: project.name, task: task.name, due: task.due, overdue: isTaskOverdue(task), projectId: project.id });
            }
            task.subTasks.forEach(subTask => {
                const subDueDate = new Date(subTask.due);
                const subDayKey = subDueDate.toISOString().split('T')[0];
                if (subDueDate.getMonth() === currentMonth.getMonth() && subDueDate.getFullYear() === currentMonth.getFullYear()) {
                    if (!tasksByDay[subDayKey]) tasksByDay[subDayKey] = [];
                    tasksByDay[subDayKey].push({ project: project.name, task: subTask.name, due: subTask.due, overdue: isTaskOverdue(subTask), projectId: project.id });
                }
            });
        });
    });
    ...
    if (calendarView === 'full') {
            ...
            for (let j = 0; j < 7; j++) {
                ...
                if (tasksByDay[dateKey]) {
                    ...
                    tasksByDay[dateKey].forEach(task => {
                        ...
                        taskSpan.className = `calendar-task ${task.overdue ? 'overdue' : ''}`;
                        taskSpan.innerHTML = `<a href="javascript:void(0)" onclick="openProjectTab('${task.projectId}');">${task.project}: ${task.task}</a>`;
                    ...
                    popup.innerHTML = `<ul>${tasksByDay[dateKey].map(task => `<li class="${task.overdue ? 'overdue' : ''}"><a href="javascript:void(0)" onclick="openProjectTab('${task.projectId}');">${task.project}: ${task.task}</a></li>`).join('')}</ul>`;
        ...
        if (sortedDays.length === 0) {
            calendarList.innerHTML = '<p>No tasks scheduled.</p>';
        } else {
            sortedDays.forEach(day => {
                const dateDiv = document.createElement('div');
                dateDiv.className = 'calendar-list-date';
                dateDiv.innerHTML = `
                    <strong>${new Date(day).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</strong>
                    <ul>${tasksByDay[day].map(task => `<li class="${task.overdue ? 'overdue' : ''}"><a href="javascript:void(0)" onclick="openProjectTab('${task.projectId}');">${task.project}: ${task.task}</a></li>`).join('')}</ul>
                `;
                calendarList.appendChild(dateDiv);
            });
        }
    }
}
...
function toggleCalendarView() {
    ...
    if (toggleBtn) toggleBtn.textContent = calendarView === 'full' ? 'Full View' : 'List View';
    renderCalendar();
}
/*
 **************************************************************************
...
function renderGanttChart(projectId) {
    ...
    const timelineDiv = document.querySelector(`[data-id="${DOM_IDS.GANTT_TIMELINE}"]`);
    if (!tbody || !timelineDiv) return;

    const allDates = [];
    project.tasks.forEach(task => {
        allDates.push(new Date(task.startDate), new Date(task.due));
        task.subTasks.forEach(subTask => {
            allDates.push(new Date(subTask.startDate), new Date(subTask.due));
        });
    });
    ...
    if (today >= minDate && today <= maxDate) {
        ...
        todayLine.style.left = `${(daysSinceStart / totalDays) * 100}%`;
    ...
    for (let i = 0; i <= totalDays; i += labelInterval) {
        ...
        label.style.left = `${(i / totalDays) * 100}%`;
    ...
    project.tasks.forEach((task, idx) => {
        const isOverdue = isTaskOverdue(task);
        const status = getTaskStatus(task, project).toLowerCase().replace(' ', '-');
        const tr = document.createElement('tr');
        const hasNotes = task.notes && task.notes.trim().length > 0;
        tr.innerHTML = `
            <td style="width: ${CONFIG.GANTT_TASK_WIDTH}" oncontextmenu="showContextMenu(event, 'task', '${project.id}', ${idx})">
                <span class="task-toggle" onclick="toggleTask('${project.id}', ${idx}, null, this)">
                    ${task.subTasks.length > 0 ? (task.expanded ? '–' : '+') : ''} 
                </span>
                ${task.name}
                <span class="notes-icon ${hasNotes ? 'has-notes' : ''}" onclick="openNotesPopup('${project.id}', ${idx}, null, event)">📝</span>
            </td>
            <td style="width: ${CONFIG.GANTT_TIMELINE_WIDTH}">
                <div class="gantt-container">
                    <div class="task-bar ${status} ${isOverdue ? 'overdue' : ''}" 
                         data-project-id="${project.id}" 
                         data-task-idx="${idx}" 
                         data-sub-idx="null" 
                         data-total-days="${totalDays}" 
                         data-min-date="${minDate.toISOString().split('T')[0]}">
                        <div class="resize-start"></div>
                        <div class="task-tooltip">${task.name}: ${task.startDate} - ${task.due}</div>
        ...
        const taskBar = tr.querySelector('.task-bar');
        const daysSinceStart = Math.floor((new Date(task.startDate) - minDate) / (1000 * 60 * 60 * 24));
        const taskDuration = Math.ceil((new Date(task.due) - new Date(task.startDate)) / (1000 * 60 * 60 * 24)) + 1;
        taskBar.style.left = `${(daysSinceStart / totalDays) * 100}%`;
        ...
        if (task.subTasks.length > 0 && task.expanded) {
            task.subTasks.forEach((subTask, subIdx) => {
                const subIsOverdue = isTaskOverdue(subTask);
                const subStatus = getTaskStatus(subTask, project).toLowerCase().replace(' ', '-');
                const subTr = document.createElement('tr');
                subTr.className = 'sub-task-row';
                const subHasNotes = subTask.notes && subTask.notes.trim().length > 0;
                subTr.innerHTML = `
                    <td style="width: ${CONFIG.GANTT_TASK_WIDTH}; padding-left: 20px" oncontextmenu="showContextMenu(event, 'subTask', '${project.id}', ${idx}, ${subIdx})">
                        ${subTask.name}
                        <span class="notes-icon ${subHasNotes ? 'has-notes' : ''}" onclick="openNotesPopup('${project.id}', ${idx}, ${subIdx}, event)">📝</span>
                    </td>
                    <td style="width: ${CONFIG.GANTT_TIMELINE_WIDTH}">
                        <div class="gantt-container">
                            <div class="task-bar ${subStatus} ${subIsOverdue ? 'overdue' : ''}" 
                                 data-project-id="${project.id}" 
                                 data-task-idx="${idx}" 
                                 data-sub-idx="${subIdx}" 
                                 data-total-days="${totalDays}" 
                                 data-min-date="${minDate.toISOString().split('T')[0]}">
                                <div class="resize-start"></div>
                                <div class="task-tooltip">${subTask.name}: ${subTask.startDate} - ${subTask.due}</div>
                                <div class="resize-end"></div>
                            </div>
                        </div>
                    </td>
                ...
                const subTaskBar = subTr.querySelector('.task-bar');
                const subDaysSinceStart = Math.floor((new Date(subTask.startDate) - minDate) / (1000 * 60 * 60 * 24));
                const subTaskDuration = Math.ceil((new Date(subTask.due) - new Date(subTask.startDate)) / (1000 * 60 * 60 * 24)) + 1;
                subTaskBar.style.left = `${(subDaysSinceStart / totalDays) * 100}%`;
                subTaskBar.style.width = `${(subTaskDuration / totalDays) * 100}%`;
            });
        }
    });
 ...
 *            Block 12 - Render Gantt Chart - script.js
 **************************************************************************
 */
...
function printGanttSchedule(projectId) {
    ...
    project.tasks.forEach(task => {
        ...
        task.subTasks.forEach(subTask => {
            ...
            subTaskRow.innerHTML = `
                ...
                <td>${subTask.due}</td>
                <td>${subTask.pic || 'None'}</td>
                <td>${subTask.completion}%</td>
            `;
            subTaskRow.className = subStatusClass;
    ...
    // Ensure visibility before printing
 ...
 *            Block 21 - Print Gantt Schedule - script.js
 **************************************************************************
 */
...
/*
 **************************************************************************
 *            BLOCK 27. Print Gantt Chart
 **************************************************************************
 */
function printGanttChart(projectId) {
    const project = projects.find(p => p.id === projectId);
    if (!project) return;

    const printContainer = document.createElement('div');
    printContainer.className = 'print-gantt-chart';
    printContainer.style.display = 'none';

    printContainer.innerHTML = `<h1>Gantt Chart: ${project.name}</h1>`;
    ...
    timelineHeader.appendChild(timeline);
    printContainer.appendChild(timelineHeader);
    ...
    project.tasks.forEach((task) => {
        ...
        tr.innerHTML = `
            ...
            <td style="width: 70%">
                <div class="gantt-container">
                    <div class="task-bar ${isOverdue ? 'overdue' : ''} ${task.completion === 100 ? 'completed' : ''}">
                    </div>
                </div>
            </td>
        `;
        ...
        if (task.expanded !== false) { // Show sub-tasks unless explicitly collapsed
            task.subTasks.forEach(subTask => {
                const subIsOverdue = isTaskOverdue(subTask);
                const subTr = document.createElement('tr');
                subTr.className = 'sub-task-row';
                subTr.innerHTML = `
                    <td style="width: 30%; padding-left: 20px">${subTask.name}</td>
                    <td style="width: 70%">
                        <div class="gantt-container">
                            <div class="task-bar ${subIsOverdue ? 'overdue' : ''} ${subTask.completion === 100 ? 'completed' : ''}">
                            </div>
                        </div>
                    </td>
                `;
    ...
    // Inject a temporary stylesheet to enforce landscape orientation
    ...
    style.textContent = `
        @media print {
            ...
            .print-gantt-chart h1 {
                font-size: 14pt;
                text-align: center;
                margin: 10px 0;
                color: #000;
            }
            .print-gantt-chart .gantt-timeline-header {
                display: block;
                width: 70%;
                margin-left: 30%;
                margin-bottom: 10px;
            ...
            .print-gantt-chart .gantt-timeline {
                position: relative;
                width: 100%;
                height: 30px;
            ...
            .print-gantt-chart .timeline-label {
                position: absolute;
                font-size: 8pt;
                color: #000;
                bottom: 2px;
            ...
            .print-gantt-chart th, .print-gantt-chart td {
                border: 1px solid #000;
                padding: 5px;
                text-align: left;
                font-size: 10pt;
            ...
            .print-gantt-chart th {
                background-color: #e0e0e0;
                font-weight: bold;
            }
            .print-gantt-chart th:nth-child(1), .print-gantt-chart td:nth-child(1) {
                width: 30%;
            }
            .print-gantt-chart th:nth-child(2), .print-gantt-chart td:nth-child(2) {
                width: 70%;
            }
            .print-gantt-chart .gantt-container {
                position: relative;
                width: 100%;
                height: 20px;
            ...
            .print-gantt-chart .task-bar.completed {
                background: #00cc00;
            }
            .print-gantt-chart .task-bar.overdue {
                background: #ff0000;
            }
            .print-gantt-chart .sub-task-row td:first-child {
                padding-left: 20px;
            }
            .print-gantt-chart tr {
                page-break-inside: avoid;
            }
            * { visibility: hidden; }
            .print-gantt-chart, .print-gantt-chart * { visibility: visible; }
        }
    `;
...
/*
 **************************************************************************
 *            END OF BLOCK 27. Print Gantt Chart
 *            Block 27 - Print Gantt Chart - script.js
 **************************************************************************
 */
