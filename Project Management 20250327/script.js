...
function isProjectOverdue(project) {
    return project.tasks.some(task => 
        ...
        task.subTasks.some(subTask => new Date(subTask.due) < new Date())
    );
}
...
function renderSubTasks(project, tbody, subTasks, parentIdx, level) {
    ...
    subTasks.forEach((subTask, idx) => {
        const isOverdue = new Date(subTask.due) < new Date();
        const tr = document.createElement('tr');
        tr.className = isOverdue ? 'overdue-row' : '';
        tr.draggable = true;
        tr.dataset.projectId = project.id;
            ...
            projects.forEach(project => {
                project.tasks.forEach(task => {
                    ...
                    if (dueDate.getFullYear() === currentDate.getFullYear() &&
                        ...
                        tasksOnDay.push({ project: project.name, task: task.name, overdue: dueDate < today, projectId: project.id });
                    }
                    task.subTasks.forEach(subTask => {
                        ...
                        if (subDueDate.getFullYear() === currentDate.getFullYear() &&
                            ...
                            tasksOnDay.push({ project: project.name, task: subTask.name, overdue: subDueDate < today, projectId: project.id });
                        }
                    });
                });
            });
...
function renderGanttChart(projectId) {
    ...
    tbody.innerHTML = '';
    project.tasks.forEach((task, taskIdx) => {
        const isOverdue = new Date(task.due) < today;
        ...
        task.subTasks.forEach((subTask, subIdx) => {
            const subIsOverdue = new Date(subTask.due) < today;
...
function startResizeStart(event) {
    ...
    function onMouseMove(e) {
        ...
        if (newLeft < 0) {
            ...
            const newTotalDays = (endDate - minDate) / (1000 * 60 * 60 * 24);
            newLeft = 0;
            initialLeft = 0;
            initialWidth = (newTotalDays / totalDays) * initialWidth;
        ...
        const snappedDays = Math.round(daysFromMinStart);
        const newStartDate = new Date(minDate);
        newStartDate.setDate(newStartDate.getDate() + snappedDays);
...
function renderTaskList() {
    ...
    projects.forEach(project => {
        project.tasks.forEach((task, taskIdx) => {
            ...
            const dateKey = dueDate.toISOString().split('T')[0];
            if (!tasksByDate[dateKey]) tasksByDate[dateKey] = [];
            tasksByDate[dateKey].push({ project: project.name, task: task.name, due: task.due, overdue: dueDate < today, projectId: project.id, taskIdx });
            task.subTasks.forEach((subTask, subIdx) => {
                const subDueDate = new Date(subTask.due);
                const subDateKey = subDueDate.toISOString().split('T')[0];
                if (!tasksByDate[subDateKey]) tasksByDate[subDateKey] = [];
                tasksByDate[subDateKey].push({ project: project.name, task: subTask.name, due: subTask.due, overdue: subDueDate < today, projectId: project.id, taskIdx: subIdx });
            });
        });
    });
...
function printGanttSchedule(projectId) {
    ...
    project.tasks.forEach(task => {
        const dueDate = new Date(task.due);
        const status = dueDate < currentDate ? 'Past Due' : 'On Track';
        const statusClass = dueDate < currentDate ? 'status-past-due' : 'status-on-track';
        ...
        task.subTasks.forEach(subTask => {
            const subDueDate = new Date(subTask.due);
            const subStatus = subDueDate < currentDate ? 'Past Due' : 'On Track';
            const subStatusClass = subDueDate < currentDate ? 'status-past-due' : 'status-on-track';
