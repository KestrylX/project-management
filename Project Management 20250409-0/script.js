...
function setupContextMenu() {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (contextMenu) {
            ...
            console.warn('Context menu element not found during click event');
        }
    });
}

function showContextMenu(event, type, projectId, taskIdx = null, subIdx = null) {
    console.log(`showContextMenu called with type: ${type}, projectId: ${projectId}, taskIdx: ${taskIdx}, subIdx: ${subIdx}`);
    ...
    try {
        if (type === 'project') {
            ...
            contextMenu.innerHTML += `
                ...
                <div class="menu-item" onclick="editProjectName('${projectId}')">Edit Project Name</div>
                <div class="menu-item" onclick="deleteProject('${projectId}')">Delete Project</div>
                <div class="menu-item" onclick="${project.isArchived ? `restoreProject('${projectId}')` : `archiveProject('${projectId}')`}">${project.isArchived ? 'Restore' : 'Archive'} Project</div>
            `;
        } else if (type === 'task') {
            contextMenu.innerHTML += `
            ...
            contextMenu.innerHTML += `
                <div class="menu-item" onclick="editTask('${projectId}', ${taskIdx}, ${subIdx})">Edit Sub-Task</div>
                <div class="menu-item" onclick="deleteTask('${projectId}', ${taskIdx}, ${subIdx})">Delete Sub-Task</div>
            `;
        } else if (type === 'addTask') {
            contextMenu.innerHTML += `<div class="menu-item" onclick="addTask('${projectId}')">Add Task</div>`;
        } else {
            console.warn(`Unknown context menu type: ${type}`);
            return;
        }

        // Position menu at mouse coordinates, adjusted for scroll
        ...
        console.log(`Context menu positioned at left: ${x}px, top: ${y}px`);

        // Adjust if menu overflows viewport
        const rect = contextMenu.getBoundingClientRect();
        if (rect.right > window.innerWidth) {
            contextMenu.style.left = `${window.innerWidth - rect.width + scrollX}px`;
        }
        if (rect.bottom > window.innerHeight) {
            contextMenu.style.top = `${window.innerHeight - rect.height + scrollY}px`;
        }
    } catch (error) {
