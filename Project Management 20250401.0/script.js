...
function isProjectOverdue(project) {
    return project.tasks.some(task => isTaskOverdue(task)); // Updated to use isTaskOverdue
}
    ...
    for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 7; j++) {
            ...
            projects.forEach(project => {
                project.tasks.forEach(task => {
                    ...
                    if (dueDate.getFullYear() === currentDate.getFullYear() &&
                        ...
                        tasksOnDay.push({ project: project.name, task: task.name, completion: task.completion, overdue: isTaskOverdue(task), projectId: project.id }); // Updated to use isTaskOverdue
                    }
                    task.subTasks.forEach(subTask => {
                        ...
                        if (subDueDate.getFullYear() === currentDate.getFullYear() &&
                            ...
                            tasksOnDay.push({ project: project.name, task: subTask.name, completion: subTask.completion, overdue: isTaskOverdue(subTask), projectId: project.id }); // Updated to use isTaskOverdue
                        }
                    });
                });
            });
            ...
            tasksOnDay.forEach(task => {
                ...
                if (task.overdue) taskDiv.className += ' overdue'; // Apply overdue class
...
function renderListCalendar(monthStart, monthEnd) {
    ...
    projects.forEach(project => {
        project.tasks.forEach(task => {
            ...
            if (dueDate >= monthStart && dueDate <= monthEnd) {
                ...
                tasksByDate[dateKey].push({ project: project.name, task: task.name, overdue: isTaskOverdue(task), projectId: project.id }); // Updated to use isTaskOverdue
            }
            task.subTasks.forEach(subTask => {
                const subDueDate = new Date(subTask.due);
                if (subDueDate >= monthStart && subDueDate <= monthEnd) {
                    ...
                    if (!tasksByDate[dateKey]) tasksByDate[dateKey] = [];
                    tasksByDate[dateKey].push({ project: project.name, task: subTask.name, overdue: isTaskOverdue(subTask), projectId: project.id }); // Updated to use isTaskOverdue
                }
            });
        });
    });
    ...
    sortedDates.forEach(date => {
        ...
        div.innerHTML = `<strong>${new Date(date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</strong>`;
        const ul = document.createElement('ul');
        tasksByDate[date].forEach(task => {
            const li = document.createElement('li');
            li.innerHTML = `<a href="#" onclick="openProjectTab('${task.projectId}')">${task.project}: ${task.task}</a>`;
            if (task.overdue) li.className = 'overdue'; // Apply overdue class
            ul.appendChild(li);
        });
        div.appendChild(ul);
...
function renderGanttChart(projectId) {
    ...
    let minDate = new Date(project.tasks[0]?.startDate || project.tasks[0]?.due || Date.now());
    ...
    project.tasks.forEach(task => {
        ...
        task.subTasks.forEach(subTask => {
            ...
            const subEnd = new Date(subTask.due);
            minDate = subStart < minDate ? subStart : minDate;
            maxDate = subEnd > maxDate ? subEnd : maxDate;
        });
    });

    const totalDays = Math.max(1, Math.ceil((maxDate - minDate) / (1000 * 60 * 60 * 24)));
    const today = new Date();
    const todayPosition = today >= minDate && today <= maxDate ? ((today - minDate) / (maxDate - minDate)) * 100 : -1;
    ...
    project.tasks.forEach((task, taskIdx) => {
        ...
        const daysFromMinStart = Math.floor((startDate - minDate) / (1000 * 60 * 60 * 24));
        const durationDays = Math.ceil((endDate - startDate) / (1000 * 60 * 60 * 24)) || 1;
        const leftPercent = (daysFromMinStart / totalDays) * 100;
        const widthPercent = (durationDays / totalDays) * 100;

        console.log(`Task ${task.name}: startDate=${task.startDate}, endDate=${task.due}`);

        // Convert YYYY-MM-DD to "MMM D" format manually
        ...
        const endDateStr = formatDate(task.due);
                ...
                <div class="gantt-container">
                    <div class="task-bar" style="left: ${leftPercent}%; width: ${widthPercent}%; background: ${isOverdue ? CONFIG.OVERDUE_COLOR : CONFIG.ON_TRACK_COLOR};" data-project-id="${projectId}" data-task-idx="${taskIdx}" data-sub-idx="null" data-total-days="${totalDays}" data-min-date="${minDate.toISOString().split('T')[0]}">
                        <div class="task-tooltip">
                            Start: ${startDateStr}<br>End: ${endDateStr}
                        </div>
        ...
        task.subTasks.forEach((subTask, subIdx) => {
            ...
            const subDaysFromMinStart = Math.floor((subStartDate - minDate) / (1000 * 60 * 60 * 24));
            const subDurationDays = Math.ceil((subEndDate - subStartDate) / (1000 * 60 * 60 * 24)) || 1;
            const subLeftPercent = (subDaysFromMinStart / totalDays) * 100;
            const subWidthPercent = (subDurationDays / totalDays) * 100;

            console.log(`Sub-Task ${subTask.name}: startDate=${subTask.startDate}, endDate=${subTask.due}`);

            const subStartDateStr = formatDate(subTask.startDate);
            const subEndDateStr = formatDate(subTask.due);
                ...
                <td style="width: ${CONFIG.GANTT_TIMELINE_WIDTH}">
                    <div class="gantt-container">
                        <div class="task-bar" style="left: ${subLeftPercent}%; width: ${subWidthPercent}%; background: ${subIsOverdue ? CONFIG.OVERDUE_COLOR : CONFIG.ON_TRACK_COLOR};" data-project-id="${projectId}" data-task-idx="${taskIdx}" data-sub-idx="${subIdx}" data-total-days="${totalDays}" data-min-date="${minDate.toISOString().split('T')[0]}">
                            <div class="task-tooltip">
                                Start: ${subStartDateStr}<br>End: ${subEndDateStr}
                            </div>
                            <div class="resize-start"></div>
                            <div class="resize-end"></div>
                        </div>
...
function renderTaskList() {
    ...
    projects.forEach(project => {
        project.tasks.forEach((task, taskIdx) => {
            ...
            const dateKey = dueDate.toISOString().split('T')[0];
            if (!tasksByDate[dateKey]) tasksByDate[dateKey] = [];
            tasksByDate[dateKey].push({ project: project.name, task: task.name, due: task.due, overdue: isTaskOverdue(task), projectId: project.id, taskIdx }); // Updated to use isTaskOverdue
            task.subTasks.forEach((subTask, subIdx) => {
                const subDueDate = new Date(subTask.due);
                const subDateKey = subDueDate.toISOString().split('T')[0];
                if (!tasksByDate[subDateKey]) tasksByDate[subDateKey] = [];
                tasksByDate[subDateKey].push({ project: project.name, task: subTask.name, due: subTask.due, overdue: isTaskOverdue(subTask), projectId: project.id, taskIdx: subIdx }); // Updated to use isTaskOverdue
            });
        });
    });
    ...
    if (sortedDates.length === 0) {
        ...
        sortedDates.forEach(date => {
            ...
            tasks.forEach(task => {
                html += `<li class="${task.overdue ? 'overdue' : ''}"><a href="#" onclick="openProjectTab('${task.projectId}')">${task.project}: ${task.task}</a></li>`; // Apply overdue class
            });
            html += '</ul></li>';
        });
    }
